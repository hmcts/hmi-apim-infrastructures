  ...
  policy_file_template = [for policy in local.policies_data.policies :
    {
      ...
      headers = can(policy.headers)  ? [for header in policy.headers :
        {
          ...
          type = header.type
          default_value = can(header.default_value) ? tostring(header.default_value) : ""
        }
      ] : []
      response = can(policy.response) ? {
        status_code = policy.response.status_code
        description = can(policy.response.description) ? tostring(policy.response.description) : ""
      } : []
      query_parameters =  can(policy.query_parameters) ? [for query_parameter in policy.query_parameters :
      {
        ...
        default_value = can(query_parameter.default_value) ? tostring(query_parameter.default_value) : ""
      }
      ] : []
      template_parameter =  can(policy.template_parameter) ? {
        ...
        default_value = can(policy.template_parameter.default_value) ? tostring(policy.template_parameter.default_value) : ""
      } : []
    }
  ]
}